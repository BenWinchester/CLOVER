...
class Energy_System:
    def __init__(self):
        ...
        self.scenario_inputs = pd.read_csv(
            self.location_filepath + "/Scenario/Scenario inputs.csv",
            header=None,
            index_col=0,
        ).round(decimals=3)
        self.kerosene_data_filepath = (
        ...
        self.simulation_storage = (
            self.location_filepath + "/Simulation/Saved simulations/"
        )
    ...
    def open_simulation(self, filename):
        ...
        """
        output = pd.read_csv(
            self.simulation_storage + str(filename) + ".csv", index_col=0
        )
        return output
    ...
    def get_load_profile(self, **options):
        ...
        loads = (
            pd.read_csv(
                self.location_filepath + "/Load/Device load/total_load.csv", index_col=0
            )
            * 0.001
        )
